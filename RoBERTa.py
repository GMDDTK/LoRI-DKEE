...
def main():
    ...
    parser.add_argument("--model_name_or_path", default=r"G:\pretrain_models\roberta-large-mnli",
                        help="RoBERTa 本地目录，例如 G:\\pretrain_models\\roberta-large-mnli")
    parser.add_argument("--save_dir", default="./ckpt_roberta")
    parser.add_argument("--max_length", type=int, default=128)
    parser.add_argument("--batch_size", type=int, default=8)  # roberta-large 比较大，默认 batch_size 调小
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--lr", type=float, default=2e-5)     # roberta-large 建议更小的学习率
    parser.add_argument("--weight_decay", type=float, default=0.01)
    parser.add_argument("--warmup_ratio", type=float, default=0.06)
    parser.add_argument("--grad_accum", type=int, default=2)  # 梯度累积，等效 batch_size = batch_size * grad_accum
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        ignore_mismatched_sizes=True  # <--- 添加这个参数
    ).to(device)
    ...
    best_f1 = -1.0
    bad_epochs = 0
    best_dir = os.path.join(args.save_dir, "roberta_best")
