...
def make_prompt_and_target(text: str, label: int):
    prompt = "Classify whether the following academic sentence is an *innovative* sentence.\n\nSentence: "+text+"\nAnswer (1 = innovative, 0 = not):"
    target = " "+str(label)
...
def main(args):
    ...
    train_loader = DataLoader(train_ds, batch_size=cfg["micro_batch_size"], shuffle=True, collate_fn=collate, num_workers=0, pin_memory=(device=="cuda"))
    model = AutoModelForCausalLM.from_pretrained(cfg["model_path"], torch_dtype=torch.float16, device_map="auto", trust_remote_code=True)
    if hasattr(model.config,"use_cache"): model.config.use_cache=False
    model = prepare_model_for_kbit_training(model)
    lora_config = LoraConfig(r=cfg["lora_r"], lora_alpha=cfg["lora_alpha"], target_modules=["q_proj","k_proj","v_proj","o_proj"], lora_dropout=cfg["lora_dropout"], bias="none", task_type="CAUSAL_LM")
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=cfg["warmup_steps"], num_training_steps=total_update_steps)
