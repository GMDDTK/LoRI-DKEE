# -*- coding: utf-8 -*-
"""
DeBERTa 微调用于文本分类（支持多/二分类）
数据：每行 "text\tlabel\tpos"（仅用前两列）
依赖：transformers, torch, sklearn, numpy, tqdm
建议 transformers 版本 >=4.20 （本地模型路径可以是 G:\pretrain_models\deberta-v3-base 等）
"""
...
def evaluate(model, loader, device):
    ...
    y_true = np.concatenate(ys) if len(ys) > 0 else np.array([], dtype=int)
    y_pred = np.concatenate(ps) if len(ps) > 0 else np.array([], dtype=int)
    if y_true.size == 0:
...
def main():
    ...
    parser.add_argument("--data_dir", default=r"classification6", help="数据目录，包含 train.txt val.txt test.txt")
    parser.add_argument("--model_name_or_path", default=r"G:\pretrain_models\deberta-v3-base",
                        help="本地 DeBERTa 模型目录或 transformers 名称（例如 G:\\pretrain_models\\deberta-v3-base）")
    parser.add_argument("--save_dir", default="./ckpt_deberta")
    ...
    # tokenizer & model
    print("Loading tokenizer from", args.model_name_or_path)
    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, use_fast=True)
    # 如果你的本地 tokenizer 需要 use_fast=False，请改为 False
    ...
    best_f1 = -1.0
    bad_epochs = 0
    best_dir = os.path.join(args.save_dir, "deberta_best")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_loss = 0.0
        optimizer.zero_grad()
        loop = tqdm(enumerate(train_loader, 1), total=len(train_loader), desc=f"Epoch {epoch}")
        for step, batch in loop:
            ...
            loop.set_postfix(train_loss=total_loss / step)

        # 验证
        val_acc, val_f1, _, _ = evaluate(model, val_loader, device)
        avg_train_loss = total_loss / max(1, len(train_loader))
        print(f"Epoch {epoch} | train_loss={avg_train_loss:.4f} | val_acc={val_acc:.4f} | val_f1m={val_f1:.4f} | time={time.time()-t0:.1f}s")
